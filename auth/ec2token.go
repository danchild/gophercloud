...
// EC2TokenOpts represents options for authenticating a user using EC2
// credentials.
type EC2TokenOpts struct {
	...
	// Not used, when a Signature is set.
	Secret string `json:"-"`
	...
	// a string. You can set the signature explicitly, when you already know
...
// ToAuthBody builds the EC2 credentials body used by both the ec2tokens and
// the s3tokens endpoints. When no Signature is set, it is calculated from the
// remaining options.
func (opts EC2TokenOpts) ToAuthBody() (map[string]map[string]any, error) {
	c, err := gophercloud.BuildRequestBody(opts, "")
	if err != nil {
		return nil, err
	}

	result := map[string]map[string]any{
		AuthEC2Token.toAuthMethod(): c,
	}
	...
	// token is only used for S3 tokens validation and will be removed when
...
func (opts EC2TokenOpts) CanReauth() bool {
...
func EC2CredentialsBuildStringToSignV2(opts EC2TokenOpts) []byte {
...
// EC2CredentialsBuildCanonicalQueryStringV4 builds a canonical query string
...
func EC2CredentialsBuildStringToSignV4(opts EC2TokenOpts, signedHeaders string, bodyHash string, date time.Time) []byte {
...
func EC2CredentialsBuildAuthorizationHeaderV4(opts EC2TokenOpts, signedHeaders string, signature string, date time.Time) string {
